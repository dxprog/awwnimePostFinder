	...
	class Awwnime {
		...
		public static function findSimilarImages($file, $limit = 5) {
			...
			if (null !== $histogram) {
				...
				$result = Db::Query('SELECT i.post_id, p.post_title, i.image_url, p.post_date, ' . $query . '0 AS distance FROM images i INNER JOIN posts p ON p.post_id = i.post_id ORDER BY distance LIMIT ' . $limit, $params);
				if ($result) {
